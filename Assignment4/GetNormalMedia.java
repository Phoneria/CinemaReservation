...
public class GetNormalMedia {
    ...
    public static void showScreen(Stage window){
        ...
        bottomButtons.getChildren().addAll(backButton, halls, okButton);
        ...
        okButton.setOnAction(event -> {
            if(CreateExtractor.getDataExtractor().getHalls().get("Avengers: Endgame").size()!=0){
                player.stop();
                ...
                GetNormalHall.showScreen(window);
            }
            else{
    ...
    public static ArrayList<String> getFutures(String nameOfTheHall){
        ArrayList<ArrayList<String>> filmInfo = CreateExtractor.getDataExtractor().getHalls().get(GetNormalFilm.nameOfTheFilm);
